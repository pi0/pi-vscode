...
- VS Code bridge bootstrap text is passed with `--append-system-prompt`; launches also pass the first workspace root's `.pi/APPEND_SYSTEM.md` as an additional `--append-system-prompt` source when present, preserving project append instructions that pi would otherwise skip once any append prompt flag is provided
